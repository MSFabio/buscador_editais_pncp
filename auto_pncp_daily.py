...
TODAY_DATE = datetime.date(2026, 8, 12)
...
SEARCH_TERMS = [
    ...
    "equipe de garçons e serviços correlatos"
]
...
def run_search_and_generate():
    log_file = os.path.join(SCRATCH_DIR, "pncp_daily.log")
    with open(log_file, "a", encoding="utf-8") as f_log:
        f_log.write(f"[{datetime.datetime.now().isoformat()}] Starting daily PNCP search...\n")

    print(f"--- PNCP Daily Search ({TODAY_DATE.strftime('%d/%m/%Y')}) ---", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"SEARCH RESULTS FOR {TODAY_DATE.strftime('%d/%m/%Y')}:", flush=True)
    ...
    # Save JSON
    json_out = os.path.join(SCRATCH_DIR, 'latest_results.json')
    with open(json_out, 'w', encoding='utf-8') as f:
        json.dump({'brasil': all_list, 'bahia': ba_list}, f, indent=2, ensure_ascii=False)
    ...
    lines_md.append(f"**Data da Busca Automática:** {TODAY_DATE.strftime('%d/%m/%Y')}  ")
    lines_md.append(f"**Profundidade:** {NUM_PAGES} Páginas por Termo de Busca  ")
    lines_md.append(f"**Termos Pesquisados (10):** {', '.join(SEARCH_TERMS)}  ")
    lines_md.append(f"**Total de Editais Abertos na Bahia:** `{len(ba_list)}` editais  ")
    lines_md.append(f"**Editais na Modalidade Pregão (Eletrônico/Presencial):** `{len(ba_pregao)}` editais  \n")

    lines_md.append("## ⚡ 1. Editais em Pregão na Bahia (BA)\n")
...
<body>
<header>
    <h1>📋 PNCP - Busca Diária de Editais Abertos na Bahia</h1>
    <div class="meta-bar">
        <b>Data da Busca Automática:</b> {TODAY_DATE.strftime('%d/%m/%Y')} &nbsp;|&nbsp;
...
"""
    ...
    with open(log_file, "a", encoding="utf-8") as f_log:
        f_log.write(f"[{datetime.datetime.now().isoformat()}] SUCCESS: Generated PDF ({pdf_file}) and Markdown ({md_file})\n")

    # Send WhatsApp Notification
    try:
        send_whatsapp_notification(ba_list)
    except Exception as e:
        print(f"[WhatsApp] Erro ao disparar notificação: {e}")

if __name__ == '__main__':
    run_search_and_generate()
