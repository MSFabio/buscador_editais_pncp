...
lines_md = []
lines_md.append("# 🛡️ Editais Abertos PNCP - Cibersegurança, Firewall, NDR & ZTNA (Brasil)")
lines_md.append(f"**Data da Busca Aprofundada:** 11/08/2026  ")
lines_md.append(f"**Profundidade:** 35 Páginas por Termo de Busca (Varredura Completa em Todo o Brasil)  ")
...
lines_md.append(f"**Total de Editais Únicos no Brasil:** `{len(items)}` editais  ")
lines_md.append(f"**Total em Pregão Eletrônico:** `{len(pregao_items)}` editais  \n")

lines_md.append("> [!NOTE]")
lines_md.append("> Todos os editais listados possuem prazo de envio de propostas ou sessão pública agendada para datas futuras a partir de 11/08/2026 em órgãos federais, estaduais e municipais de todo o Brasil.\n")
...
lines_md.append("### Distribuição Geográfica (Top Estados)")
...
lines_md.append("## ⚡ 1. Principais Editais em Pregão Eletrônico (Brasil)\n")

for idx, item in enumerate(pregao_items[:60], 1):
...
<head>
<meta charset="UTF-8">
<title>PNCP - Cibersegurança, NDR, ZTNA e Firewall (Brasil)</title>
<style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap');
    @page {{ size: A4; margin: 12mm; }}
    body {{
        font-family: 'Inter', -apple-system, sans-serif;
        color: #0f172a; background: #ffffff; font-size: 11.5px; line-height: 1.45; margin: 0; padding: 0;
    }}
    header {{ border-bottom: 2px solid #0284c7; padding-bottom: 10px; margin-bottom: 14px; }}
    h1 {{ color: #0284c7; font-size: 19px; margin: 0 0 6px 0; font-weight: 700; }}
    ...
    h2 {{ font-size: 14px; color: #0f172a; border-left: 4px solid #0284c7; padding-left: 8px; margin: 20px 0 10px 0; font-weight: 700; }}
    .card {{
    ...
    .card-title {{ font-weight: 700; font-size: 12px; color: #0f172a; margin: 0; }}
    ...
    .links-bar a {{ color: #0284c7; text-decoration: none; font-weight: 500; margin-right: 12px; }}
</style>
</head>
<body>

<header>
    <h1>🛡️ PNCP - Cibersegurança, Firewall, NDR & ZTNA (Brasil)</h1>
    <div class="meta-bar">
        <b>Data da Busca:</b> 11/08/2026 &nbsp;|&nbsp;
        <b>Profundidade:</b> 35 Páginas por Termo (Busca Nacional) &nbsp;|&nbsp;
        <b>Fonte:</b> Portal Nacional de Contratações Públicas
    </div>
    <div class="summary-box">
        <div class="stat-item"><div class="stat-val">{len(items)}</div><div class="stat-lbl">Total Editais em Cibersegurança</div></div>
        ...
        <div class="stat-item"><div class="stat-val" style="color: #475569;">{len(outros_items)}</div><div class="stat-lbl" style="color: #475569;">Concorrência & Outras</div></div>
    </div>
</header>

<h2>⚡ Editais Selecionados de Cibersegurança & TI ({len(pregao_items)} Pregões Eletrônicos)</h2>
"""

for idx, item in enumerate(pregao_items[:80], 1):
    orgao = item.get('orgao') or 'Não informado'
    uf = item.get('uf') or 'DF'
    muni = item.get('municipio') or uf
    title = item.get('title') or f"Edital {item.get('control_num')}"
