...
TODAY_DATE = datetime.date(2026, 8, 18) # Today: August 18, 2026
...
def main():
    print(f"--- PNCP Search for 18/08/2026 ({NUM_PAGES} pages x {len(SEARCH_TERMS)} terms) ---", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"NEW PNCP SEARCH RESULTS (18/08/2026):", flush=True)
    ...
    # Save JSON
    json_out = os.path.join(SCRATCH_DIR, 'aug18_bahia_results.json')
    ...
    lines_md.append(f"**Termos Pesquisados (9):** {', '.join(SEARCH_TERMS)}  ")
    ...
    lines_md.append("> Todos os editais listados abaixo possuem prazo de envio de propostas ou sessão pública agendada para datas futuras a partir de 18/08/2026.\n")
