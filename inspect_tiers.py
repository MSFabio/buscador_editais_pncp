...
print(f"Total Bahia Editais: {len(ba_items)}")
print(f"Tier 1 (Até 100km): {len(tier1)} editais")
print(f"Tier 2 (101km a 200km): {len(tier2)} editais")
print(f"Tier 3 (201km a 300km): {len(tier3)} editais")
print(f"Tier 4 (Acima de 300km): {len(tier4)} editais")

print("\n--- MUNICÍPIOS ATÉ 100 KM ---")
for i in set(f"{x['municipio_ref']} ({x['distance_km']} km)" for x in tier1):
    print(" -", i)

print("\n--- MUNICÍPIOS 101 A 200 KM ---")
for i in set(f"{x['municipio_ref']} ({x['distance_km']} km)" for x in tier2):
    print(" -", i)

print("\n--- MUNICÍPIOS 201 A 300 KM ---")
for i in set(f"{x['municipio_ref']} ({x['distance_km']} km)" for x in tier3):
    print(" -", i)

print("\n--- MUNICÍPIOS ACIMA DE 300 KM ---")
for i in set(f"{x['municipio_ref']} ({x['distance_km']} km)" for x in tier4):
    print(" -", i)
